...
export const guestRegister = async (req, res) => {
  const { first_name, last_name, email, phone_number, password, items } = req.body;
  try {
    await sql.begin(async (sql) => {
      ...
      const hashedPassword = password ? await bcrypt.hash(password, 10) : await bcrypt.hash(uuidv4(), 10);

      const [newUser] = await sql`
        INSERT INTO users (first_name, last_name, username, email, password, phone_number, first_order, is_temporary)
        VALUES (${first_name}, ${last_name}, ${username}, ${email}, ${hashedPassword}, ${phone_number}, false, true)
        RETURNING id, first_name, last_name, username, email, phone_number, is_admin
      `;

      // Create cart
      const [newCart] = await sql`INSERT INTO cart (user_id, total) VALUES (${newUser.id}, 0) RETURNING id`;

      // Copy items to new cart (simplified; adapt your addToCart logic)
      for (const item of items || []) {
        await sql`
          INSERT INTO cart_items (cart_id, user_id, variant_id, bundle_id, quantity, price, color_name, size_name)
          VALUES (${newCart.id}, ${newUser.id}, ${item.variant_id || null}, ${item.bundle_id || null}, ${item.quantity || 1}, ${item.price || 0}, ${item.color || null}, ${item.size || null})
        `;
      }
