  ...
  try {
          ...
          COALESCE(
            json_agg(
              json_build_object(
                ...
                'images', (
                  SELECT COALESCE(json_agg(image_url ORDER BY COALESCE(pi.sort_order, 0) ASC, pi.is_primary DESC, pi.id ASC), '[]'::json)
    ...
    const [bundle] = await sql`
      SELECT 
        ...
        COALESCE(
          (SELECT COALESCE(json_agg(image_url ORDER BY COALESCE(bi2.sort_order, 0) ASC, bi2.is_primary DESC, bi2.id ASC), '[]'::json) FROM bundle_images bi2 WHERE bi2.bundle_id = b.id),
          '[]'::json
        ) AS images
      FROM bundles b
      LEFT JOIN bundle_items bi ON b.id = bi.bundle_id
  ...
  try {
    ...
    const [sibling] = await sql`
      SELECT b.id, b.bundle_price AS price, b.bundle_type,
        COALESCE((SELECT COALESCE(json_agg(image_url ORDER BY COALESCE(bi.sort_order, 0) ASC, bi.is_primary DESC, bi.id ASC), '[]'::json) FROM bundle_images bi WHERE bi.bundle_id = b.id), '[]'::json) AS images
      FROM bundles b
