...
CREATE TABLE public.bundles (
    ...
    CONSTRAINT bundles_bundle_type_check CHECK (((bundle_type)::text = ANY ((ARRAY['3-in-1'::character varying, '5-in-1'::character varying])::text)))
);
...
CREATE TABLE public.discounts (
    ...
    CHECK (discount_type::text = ANY (ARRAY['percentage'::character varying, 'fixed'::character varying]::text[]))
);
...
CREATE TABLE public.review_votes (
    ...
    CHECK (vote_type::text = ANY (ARRAY['helpful'::character varying::text, 'not_helpful'::character varying::text]))
);
