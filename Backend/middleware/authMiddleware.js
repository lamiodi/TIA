...
// but adds user info to request if authenticated
export const optionalAuth = async (req, res, next) => {
  const authHeader = req.headers.authorization;
  console.log('optionalAuth: Auth Header for', req.url, ':', authHeader);
  ...
  try {
    ...
    if (!secret) {
      console.error('optionalAuth: JWT_SECRET not defined for', req.url);
      req.user = null;
      req.isGuest = true;
      return next();
    }
    ...
    console.log('optionalAuth: Decoded token for', req.url, ':', decoded);
    ...
    if (!user) {
      console.error('optionalAuth: User not found for id:', decoded.id, 'on', req.url);
      req.user = null;
      req.isGuest = true;
      return next();
    }
    ...
    req.isGuest = false;
    console.log('optionalAuth: Authenticated user for', req.url, ':', req.user);
    next();
  } catch (err) {
    console.error('optionalAuth: Token verification failed for', req.url, ':', err.message);
